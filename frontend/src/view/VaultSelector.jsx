...
function VaultSelector() {
  ...
  const handleCreateVault = async (onClose) => {
    ...
    try {
      const result = await dispatch(createVault({ name: vaultName.trim() })).unwrap();
      setVaultName('');
      onClose();
      // Optionally navigate to the new vault
      navigate(`/vaults/${result.id}`);
    } catch (error) {
    ...
    <VStack spacing={8} align="stretch">
      <Feature name="header">
        <Box display="flex" alignItems="center" justifyContent="space-between" pb={2}>
          ...
          >
            <PlusIcon width={20} className="hover:scale-110 transition-transform" />
          </Button>
        </Box>
        <Separator />
              ...
              {({ onClose }) => (
                <>
                  <Box 
                    ...
                    zIndex={999}
                  />
                  <Box position="fixed" top="50%" left="50%" transform="translate(-50%, -50%)" zIndex={1000} bg={bg} borderRadius="xl" boxShadow="xl" width="90%" maxWidth="400px" onClick={(e) => e.stopPropagation()}>
                    ...
                    <VStack spacing={3} p={4} align="stretch">
                      <Text fontSize="sm" align="center" textAlign="center" fontWeight="medium" mb={2}>Create a new vault</Text>
                      <Input 
